...
═══════════════════════════════════════════════════════════════════════════
    ...
    Complete 3-Algorithm Systems with Performance Analysis
    
    Total Systems: 12 Complete Scenarios
...
try:
    import numpy as np
    HAS_NUMPY = True
    print("  ✓ NumPy loaded (Hill Cipher system enabled)")
except ImportError:
    HAS_NUMPY = False
    print("  ⚠ NumPy not available (Hill Cipher system disabled)")
...
class LegacyBankingSystem:
    ...
    def verify_transaction(self, txn_id, customer_id):
        """Verify transaction"""
        txn = next((t for t in self.transactions if t['id'] == txn_id), None)
        if not txn:
            return False
...
def menu_legacy_banking():
    ...
    while True:
        ...
        if choice == '1':
            ...
            txn_id = int(input("Transaction ID: "))
            cust = input("Customer ID: ")
            system.verify_transaction(txn_id, cust)
        elif choice == '4':
            for t in system.transactions:
                print(f"  [{t['id']}] Hash: {t['hash'][:16]}...")
        elif choice == '5':
...
if HAS_NUMPY:
    def matrix_mod_inv(matrix, modulus):
        ...
        if det_inv is None:
            raise ValueError(f"Matrix determinant {det} is not invertible mod {modulus}")
    ...
    class HillHybridSystem:
        ...
        def _hill_decrypt(self, ciphertext, key_matrix):
        ...
        def send_message(self, sender, recipient, message, key_matrix_str):
            ...
            # Hill encryption
            start = time.time()
            encrypted_msg = self._hill_encrypt(message, key_matrix)
            ...
            start = time.time()
            cipher_rsa = PKCS1_OAEP.new(self.rsa_keys[recipient]['public'])
            encrypted_key = cipher_rsa.encrypt(key_matrix_str.encode())
            rsa_time = time.time() - start
            self.performance.record('RSA_Encrypt', rsa_time)
        ...
        def read_message(self, user_id, msg_id):
            ...
            start = time.time()
            cipher_rsa = PKCS1_OAEP.new(self.rsa_keys[user_id]['private'])
            key_matrix_str = cipher_rsa.decrypt(base64.b64decode(msg['encrypted_key'])).decode()
            rsa_time = time.time() - start
            self.performance.record('RSA_Decrypt', rsa_time)
            ...
            # Hill decryption
            start = time.time()
            decrypted_msg = self._hill_decrypt(msg['encrypted_msg'], key_matrix)
            hill_time = time.time() - start
            self.performance.record('Hill_Decrypt', hill_time)

            # SHA-256 verification
            start = time.time()
            # Need to find original message length to remove padding
            original_msg = decrypted_msg # This is a simplification; padding removal is tricky
            computed_hash = SHA256.new(original_msg.encode()).hexdigest()
            # A proper system would store length, but for this demo we'll hash the padded msg
            computed_hash_with_padding = SHA256.new(decrypted_msg.encode()).hexdigest()
            verified = (computed_hash == msg['hash'] or computed_hash_with_padding == msg['hash'])
            sha_time = time.time() - start
            self.performance.record('SHA256_Verify', sha_time)
...
def master_menu():
    ...
    while True:
        ...
        print("="*70)
        print("\nSELECT SYSTEM (All are complete 3-algorithm systems):")
        print()
        ...
        # Conditional options based on libraries
        if HAS_PAILLIER:
        ...
        print()
        print(" 13. Universal Algorithm Benchmark")
        print(" 14. About This Script")
        print(" 15. Exit")
        print("-"*70)
        
        choice = input("\nSelect system (1-15): ").strip()
        ...
        if choice == '1':
            ...
            print("  Version: 2.0 (All Combinations)")
            print("  Total Systems: 12 complete scenarios")
            print("  Total Algorithms: 10 (DES, 3DES, AES, RSA, Rabin, ElGamal,")
            print("                       Paillier, Hill, SHA-1, SHA-256, SHA-512, MD5)")
